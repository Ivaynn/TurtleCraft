...
# Get current value
execute store result score $new_value turtlecraft.tmp run data get storage turtlecraft:tmp edit_arg.line
...
# Check bounds
execute unless score $new_value turtlecraft.tmp matches 0.. run scoreboard players operation $new_value turtlecraft.tmp *= #_1 turtlecraft.math


# Save changes
execute store result storage turtlecraft:tmp list_start[-1].A.line int 1 run scoreboard players get $new_value turtlecraft.tmp

...
execute if score show_warnings turtlecraft.options matches 1 if score $cancel turtlecraft.tmp matches 1 run tellraw @a ["",{"text":"WARNING: ","color":"gold"},{"text":"Maximum Turtle count reached!","color":"gray","hover_event":{action:"show_text",value:[{"text":"To add more, remove some Turtles\nor increase this limit","color":"gray"}]}}]

...
execute store result score $turtle_lock tc.tmp run data get entity @e[limit=1,type=minecraft:llama,tag=tc.tmp,tag=tc.body] ArmorItems[0].tag.Instructions[0].Locked 1
execute if score $turtle_lock tc.tmp matches 1 run data modify storage tc:tmp uuid.owner set from entity @e[limit=1,type=minecraft:llama,tag=tc.tmp,tag=tc.body] ArmorItems[0].tag.Owner.UUID

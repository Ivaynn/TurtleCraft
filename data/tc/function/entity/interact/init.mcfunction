...
execute if score $turtle_busy tc.tmp matches 0 if score locked_turtles tc.options matches 1 unless data entity @e[limit=1,type=minecraft:llama,tag=tc.tmp,tag=tc.body] ArmorItems[0].components."minecraft:custom_data".tc.Owner run scoreboard players set $turtle_lock tc.tmp 0
